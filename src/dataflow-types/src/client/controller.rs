...
use crate::client::{
    ComputeClient, ComputeCommand, ComputeInstanceId, ComputeResponse, InstanceConfig, Response,
    StorageClient, StorageResponse,
};
    ...
    T: Timestamp + Lattice,
{
    pub async fn create_instance(
        ...
        fn make_client<T>(_config: InstanceConfig) -> Box<dyn ComputeClient<T>> {
            todo!()
        }
        let mut client: Box<dyn ComputeClient<T>> = make_client(config);
        client
...
impl<T> Controller<T> {
    ...
    pub fn new(storage_client: Box<dyn StorageClient<T>>) -> Self {
        Self {
